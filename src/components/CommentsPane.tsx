...
export default function CommentsPane({ postId, onCommentsCountChange }: CommentsPaneProps) {
  ...
  const [repliesData, setRepliesData] = useState<{ [commentId: string]: Reply[] }>({});
  ...
  // Real-time listeners for replies for each comment
  useEffect(() => {
    const unsubscribes: (() => void)[] = [];

    comments.forEach((comment) => {
      // Setup reply listeners for active comments
  ...
  const handlePostReply = async (commentId: string) => {
    ...
    try {
      ...
      await setDoc(customReplyRef, {
        id: newReplyId, // This matches the document ID and satisfies firestore rules perfectly!
        content: text,
        likesCount: 0,
        createdAt: serverTimestamp(),
      });
      ...
      handleFirestoreError(error, OperationType.WRITE, `posts/${postId}/comments/${commentId}/replies`);
    } finally {
  ...
  const handleCommentLike = async (commentId: string) => {
    ...
    try {
      ...
      // Revert local storage state if failed
  ...
  const handleReplyLike = async (commentId: string, replyId: string) => {
    ...
    try {
      ...
      handleFirestoreError(error, OperationType.UPDATE, `posts/${postId}/comments/${commentId}/replies/${replyId}`);
    }
  };
  ...
  const handleReplyInputChange = (commentId: string, val: string) => {
    setReplyInputs((prev) => ({
      ...prev,
      [commentId]: val,
    }));
      ...
      <div className="space-y-4">
        {comments.length === 0 ? (
          ...
          comments.map((comment, index) => {
            const commentLiked = isCommentLiked(`${postId}_${comment.id}`);
            const commentReplies = repliesData[comment.id] || [];
            const showReplyInput = expandedReplies[comment.id];
            
            // Generates a deterministically matching unique-looking designator tag based on ID
